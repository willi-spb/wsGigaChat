...
TonSendCommandEvent=procedure(Sender:TObject; aComChar:char; const aComName,aComDataStr:String; acDt:TDatetime; const aErrStr:string) of object;
...
/// </summary>
 TCodeTraceManager=class(TObject)
  private
   ...
   FEnabled:boolean;
   FonSendCommand:TonSendCommandEvent;
  ...
  public
   ...
   property onSendCommand:TonSendCommandEvent read FonSendCommand write FonSendCommand;
 end;
...
procedure TCodeTraceManager.ClearSendErrors;
begin
  F_SendStageCount:=0;
end;
  ...
  if FItemsEnabled then
   try
     ...
     if (FMaxItemsCount>0) and (FItems.Count>FMaxItemsCount) then
       ...
       L_err:='modItemsERROR > '+E.ClassName+' : '+E.Message;
   end;
   /// send
   ///  if repeat errors in Connect and Send great 5 - then DISABLE send!!
  if F_SendStageCount<5 then
    try
     if FSocket.FUdpClient.Connected=false then
      ...
      begin
        L_err:='udpClientERROR > '+E.ClassName+' : '+E.Message;
        Inc(F_SendStageCount);
      end;
    end;
   ///
 finally
  try
    if Length(LCommand)=0 then LCommand:='*';
    if (Assigned(LItem)) and (Assigned(FonSendCommand)) then
       FonSendCommand(Self,LCommand[1],LItem.CommandName,LItem.MessData,LItem.DTime,L_err);
   finally
